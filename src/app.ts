...
// Configuration
const WEBHOOK_URL       = 'SET URL LIKE https://hooks.slack.com/services/****/****/****';
const SLACK_USERNAME    = 'cronsheet';
const SLACK_ICON_EMOJI  = ':clock3:';
// ... END Configuration

const COLUMN_DATE          = 1;
const COLUMN_HOUR          = 2;
const COLUMN_START_MESSAGE = 3;
...
function doPost() {
  ...
  for (var r = 2; r <= maxRows; r++) {
    ...
    var date = row.getCell(1, COLUMN_DATE).getValue(),
        hour = row.getCell(1, COLUMN_HOUR).getValue();
    if (date === '' && hour === '') break;
    ...
    for (var c = COLUMN_START_MESSAGE; c <= row.getLastColumn(); c++) {
      if (headerValues[c-1].toString() === '') break;
...
class SlackMessage {
  ...
  constructor(config: { username: string; icon_emoji: string; header: string; }) {
    this.username   = config.username;
    this.icon_emoji = config.icon_emoji;
    this.header     = config.header;
  }

  addField(title: string, value: string) {
  ...
  toPayload(): Object {
    var payload = {
      attachments: [
        {
          fallback: this.fields.map((p) => p.title + '=' + p.value).join('; '),
          pretext:  this.header,
          fields:   this.fields
        }
      ],
...
function conditionMatches(date: any, hour: any, now: Date): boolean {
  if (!dateMatches(date, now)) return false;

  if (!hourMatches(hour, now)) return false;

  return true;
}

function dateMatches(date: any, now: Date): boolean {
  ...
  var days: string[] = date.toString().toLowerCase().split(/[, ]+/);
...
function hourMatches(hour: any, now: Date): boolean {
